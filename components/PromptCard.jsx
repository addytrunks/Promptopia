  ...
  return (
    <div className='prompt_card'>
      <div className='flex justify-between items-start gap-5'>
        ...
        <p className='text-sm text-green-600 cursor-pointer' onClick={handleCopy}>{copied === post.prompt ? 'Copied':'Copy' }</p>
      </div>

      <p className='my-4 font-satoshi text-sm text-gray-700'>{post.prompt}</p>
      <p className='font-inter text-sm blue_gradient cursor-pointer' onClick={() => handleClick && handleTagClick(post.tags)}>#{post.tags}
      </p>

      {session?.user.id == post.creator._id && pathName ==='/profile' && (
         <div className='mt-5 flex-center gap-4 border-t border-gray-100 pt-3'>
            <p className='text-sm green_gradient font-inter cursor-pointer' onClick={handleEdit}>Edit</p>
